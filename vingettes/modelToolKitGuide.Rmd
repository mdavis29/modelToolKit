...
Apply levels from one data frame to another, and use encoding to create an encoded matrix 
```{r}
...
dmv<-dummyVars(~., mydata, n = c(2,2))
str(mydata)
str(levelMatcher(dmv, newData = mydata1))
mydata1.matched<-(levelMatcher(newData = mydata1, oldData = mydata))
mydata1.dmv<-(predict(dmv,mydata1.matched))
mydata1.dmv[is.na(mydata1.dmv)]<-0
head(mydata1.dmv)
```                
